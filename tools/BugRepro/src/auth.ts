...
export async function permanAuth(page: Page): Promise<void> {
  ...
  await modal.locator(process.env.PERMAN_USERNAME_SELECTOR ?? "").fill(process.env.BASIC_USER ?? "");
  await modal.locator(process.env.PERMAN_PASSWORD_SELECTOR ?? "").fill(process.env.BASIC_PASS ?? "");
