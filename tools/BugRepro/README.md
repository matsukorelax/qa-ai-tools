...
8端末・10環境を担当するQA業務では、バグ発見時に端末・OS・環境ごとに手動で再現確認が必要で、そのスイッチコストが高かった。BugReproはそのコストを削減するために作った。

## できること

- URLとバグ概要を渡すだけでPlaywrightのテストコードを自動生成する
- 生成したコードをそのまま実行して再現確認まで完了する（webview対応）
- Android / iOS対応は開発中 🚧
